...
export default class RefreshTokenUseCase implements IUseCase<string> {
  constructor(
    ...
    private readonly setRefreshTokenCookie: (
      refreshToken: string,
      res: Response,
      key: string,
    ) => void,
    private readonly clearRefreshTokenCookie: (
      res: Response,
      key: string,
    ) => void,
  ) {}
  ...
  async call(userId: string, token: string, res: Response): Promise<string> {
    ...
    const refreshExpiresIn = ms(
      jwtConfig.refreshToken.expiresIn as StringValue,
    );
    if (!refreshExpiresIn) {
    ...
    const refrestTokenInstance = mapper.map(
      refreshTokenRequest,
      RefreshTokenRequest,
      RefreshToken,
    );
